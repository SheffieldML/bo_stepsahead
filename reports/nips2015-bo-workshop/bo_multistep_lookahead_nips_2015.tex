...
\begin{abstract}
    ...
    Our novel algorithm, \us, permits the consideration of dozens of evaluations into the future.  This is done by approximating the ideal \emph{look-ahead} loss function. We show that the far-horizon planning thus enabled leads to substantive performance gains in empirical tests. 
\end{abstract}
...
where the subscript in $\Lambda$ refers to the fact that we are considering one future evaluation.  he next evaluation is located   where $\Lambda_1(\latentVector_*| \I_0) $ gives the minimum value. This point can  be obtained by any gradient descent algorithm since analytical expressions for the loss and its gradients are explicitly available \citep{osborne_bayesian_2010}. 
...
\end{equation}
where  $h_i(\dataVector) = \mathbb{I}\{\dataScalar_i>\eta\}$ and $$t_{j,i}(\dataVector)= \left\{ \begin{array}{lcl}
...
To compare the methods we used the `gap' measure of performance \citep{Huang:2006}.
