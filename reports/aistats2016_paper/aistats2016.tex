...
\begin{abstract}
    ...
    Our novel algorithm, \us, permits the consideration of dozens of evaluations into the future.  This is done by approximating the ideal \emph{look-ahead} loss function, which is expensive to evaluate, by a cheaper alternative in which the future steps of the algorithm are simulated beforehand. An Expectation Propagation algorithm is used to compute the expected value of the loss.
...
\begin{itemize}
\item Make explicit the conection of the formulation with Snoek et al. (2012)
\item Of course, we did mean that there aren’t general use Bayesian optimisation packages available with non-myopic loss functions. The typo has been corrected.
\item We agree that to compare the different methods the standard deviations needs to be included in the experiments. Due to space reasons, such values have been included in the suppl. materials for all the experiments. We anticipate that all standard deviations are quite small and that the differences among the studied methods are significant as currently described in the text. Note that this is not surprising since standard deviations at the end of the experiments (as reported in the table) are typically small: they correspond to phases of the optimization close to convergence. In most BO results of the literature one can observe this behaviour.
\item extra results in mail
\item Regarding the computational complexity of GLASSES, the parameter q, as used in the general notation of the paper, determines the input dimension of the function. We have clarified this point in Section 3.3
\end{itemize}
...
For example, consider the case in which we have a budget of two evaluations with which to optimise a function $f(x)$ over the domain $\inputSpace = [0, 1] \subset \Re$. 
...
\subsection{Standard deviation of the `gap' measures}
...
\midrule
func = fourier(sd=.1)             &  0.1502 &  0.1442 &  0.1221 &  0.0707 &  0.1429 &  0.1749 &  0.0862 &  0.0499 \\
func = cosines(sd=.1)             &  0.0377 &  0.0368 &  0.0548 &  0.0394 &  0.0389 &  0.0417 &  0.0633 &  0.0135 \\
func = branin(sd=.1)              &  0.0060 &  0.0121 &  0.0004 &  0.0020 &  0.0146 &  0.0036 &  0.0005 &  0.0030 \\
func = sixhumpcamel(sd=.1)        &  0.0065 &  0.0199 &  0.0063 &  0.0080 &  0.0104 &  0.0080 &  0.0096 &  0.0092 \\
func = mccormick(sd=.1)           &  0.0093 &  0.0091 &  0.0242 &  0.0152 &  0.0135 &  0.0128 &  0.0116 &  0.0129 \\
func = dropwave(sd=.1)            &  0.0473 &  0.0595 &  0.0558 &  0.0293 &  0.0320 &  0.0238 &  0.0229 &  0.0407 \\
func = powers(sd=.1)              &  0.0073 &  0.0073 &  0.0071 &  0.0186 &  0.0063 &  0.0147 &  0.0059 &  0.1415 \\
func = ackley(input\_dim=2,sd=.1)  &  0.0491 &  0.0103 &  0.1197 &  0.1061 &  0.1349 &  0.1005 &  0.1171 &  0.1637 \\
func = ackley(input\_dim=5,sd=.1)  &  0.0196 &  0.0181 &  0.1146 &  0.1809 &  0.1433 &  0.1401 &  0.1779 &  0.1361 \\
func = ackley(input\_dim=10,sd=.1) &  0.0015 &  0.0016 &  0.1519 &  0.0011 &  0.0020 &  0.0019 &  0.1386 &  0.1209 \\
func = alpine2(input\_dim=2,sd=.1) &  0.0957 &  0.0903 &  0.1132 &  0.0848 &  0.0534 &  0.0822 &  0.0878 &  0.0439 \\
func = alpine2(input\_dim=5,sd=.1) &  0.0679 &  0.0577 &  0.0579 &  0.0835 &  0.0878 &  0.0808 &  0.0777 &  0.0814 \\
\bottomrule
\end{tabular}\caption{Results for the average `gap' measure (5 replicates) across different functions.  \el-k is the expect loss function computed with $k$ steps ahead at each iteration. \us is the \us algorithm, \mpi is the maximum probability of improvement and \lcb is the lower confidence bound criterion. The best result for each function is bolded. In italic, the cases in which a non-myopic loss outperforms the myopic  loss are highlighted.}\label{table:comparision}
\end{center}
\end{table*}
