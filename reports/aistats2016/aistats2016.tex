...
To study the validity of our approximation we choose a variety of functions with a range of dimensions and domain sizes. See Table~\ref{table:test_functions} for details. We test the \us algorithm using 2, 3, 5, and 10 steps look-ahead to compute the loss function. We also include the full \us approach in which the number of remaining iterations is used as the number of steps-ahead that we use to compute the expected loss. Each problem is solved a unique initilization with a number of data points equal to 5. This allows us to compare the average performance of each method on each problem. As baseline we used the myopic expected loss. for comparison purposes, all expected losses were optimised using the dividing rectangles algorithm \direct \citep{Jones1993}.
...
\midrule
SinCos &&& &&&&&\\ 
Cosines           &  0.8637 &  0.8704 &  0.8161 &  0.8423 &  0.8118 &  0.7946 &  0.7477 &  \textbf{0.8722} \\
...
Mccormick           &  0.9514 &  0.9326 &  0.9055 &  0.9139 &  0.9189 &  0.9283 &  0.9389 &  \textbf{0.9424} \\
Dropwave            &  0.7308 &  0.7413 &  0.7667 &  0.7237 &  0.7555 &  0.7293 &  0.6860 &  \textbf{0.7740} \\
Beale &&& &&&&&\\ 
Powers              &  0.2177 &  0.2167 &  0.2216 &  0.2428 &  0.2372 &  0.2390 &  0.2339 &  \textbf{0.3670} \\
Ackley-2 &  0.8230 &  0.8975 &  0.7333 &  0.6382 &  0.5864 &  0.6864 &  0.6293 &  0.7001 \\
Ackley-5  &&& &&&&&\\ 
Alpine2-2 &  0.8628 &  0.8482 &  \textbf{0.7902} &  0.7467 &  0.5988 &  0.6699 &  0.6393 &  0.7807 \\
Alpine2-5 &  0.5221 &  0.6151 &  \textbf{0.7797} &  0.6740 &  0.6431 &  0.6592 &  0.6747 &  0.7123 \\
\bottomrule
\end{tabular}\caption{Results for the average `gap' measure across different functions and methods in 5 replicates.  GL-k represents that the expect loss function has been computed k steps ahead at each iteration. GL-full represents the full \us algorithm with a number of steps ahead in each iteration equal to the number of remaining evaluations. The best result when comparing the myopic expected loss with the \us method with different number of steps ahead is bolded. The  MPI and GP-LCB are included for reference.}
% \end{center}
...
\end{equation}
where $y(\cdot)$ represents the evaluation of the objective function, $y(\latentVector_{opt})$ is the global minimum, and $\latentVector_{first}$ and $\latentVector_{best}$ are the first and best evaluated point, respectively. For every method tested, the initial point $\latentVector_{first}$ was chosen to be the the best of the generated points used to initialise the model. Also, since the evaluations of $f$ are perturbed by Gaussian noise in the experiments some evaluations can be lower than the local optimum and the gap become larger than one. To compensate this effect, we normalise the gaps measures across each problem by dividing all of them by the best found gap. This way, the best technique has always a normalised gap that is equal to one.

For comparative purposes we used other two loss functions are commonly used in the literature. In particular we used   the Maximum probability of Improvement (MPI) and the GP Upper confidence bound (GP-UCB). In this later case we set the parameter that balances exploration and exploitation to 1. See \citep{Snoek*Larochelle*Adams_2012} for details on these loss functions.  

Table \ref{table:comparision} shows the comparative across different functions and methods. None of the methods is universally but the \us algorithm is the best in XX of the XX cases. In xx cases the full glasses approach is the best of all methods. Specially interesting is the case of the McCormick and the Powers function. In these two cases, to increase the number of steps ahead used to compute the loss consistently improve the obtained results.

The myopic shows the best performance in some of the experiments. However, when this happens, the full \us method typically shows a very similar performance. 
...
Some interesting issues will be addressed in the future. Perhaps the most relevant is the way the non-myopic loss function is optimised.  Although in this work we have used \direct to address this step, specific and more efficient gradient-based methods will be considered. Also, the use of non-stationary kernels, and extensions of the methods to high dimensional problems and efficient was of sampling many steps ahead will also be analysed in the near future.
