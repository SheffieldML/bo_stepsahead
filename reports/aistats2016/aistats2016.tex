...
\bottomrule
\end{tabular}\caption{Results for the average `gap' measure (5 replicates) across different functions.  \el-k represents the expected loss function computed for k steps ahead at each iteration. \us represents the full \us algorithm, \mpi is the maximum probability of improvement and \lcb is the lower confidence bound criterion. The best result for each function is bolded. The cases in which a non-myopic loss outperforms the myopic expected loss are highlighted in italic.}\label{table:comparision}
\end{center}
\end{table*}
