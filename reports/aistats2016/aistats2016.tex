...
The goal of this work is \emph{to propose an approximation to Eq.~(\ref{eq:expected_nonmyopic_loss}) with a minimal computational burden that will allow to relieve the myopic aspect of Bayesian optimization in a wide class of problems}. The key aspect our approach is to split the recursive optimization marginalization loop in Eq.~(\ref{eq:expected_nonmyopic_loss}) in two independent optimization-marginalizaiton that jointly act on all the future steps.
...
\subsection{Predicting BO future steps}

[EXPLAIN HERE HOW WE PREDICT THE FUTURE STEPS OF THE ALGORITHM: batch method of final alternative]

%\begin{figure*}[t!]
%    \centering
%    \begin{subfigure}%{0.5\textwidth}
%        \centering
%        \includegraphics[height=2.5in]{predicted_locations1.png}
%    \end{subfigure}%
%    \begin{subfigure}%{0.5\textwidth}
%        \centering
%        \includegraphics[height=2.5in]{predicted_locations2.png}
...
The goal of this section is to propose a way of computing Eq.~(\ref{eq:expected_nonmyopic_loss}) when the the putative input $\bx_*$ and the future evaluations $\bx_2,\dots,\bx_n$, that we denote by $\future_{n}(\bx_*)$, are available. Ideally, such evaluations should come from some oracle able to predict the algorithm search. In practice, we will use the method presented in the previous section. 

The expected loss Eq.~(\ref{eq:expected_nonmyopic_loss}) when $\future_{n}(\bx_*)$ is available results in
\begin{equation}
\Lambda_n(\bx_*|\I_0, \future_{n}(\bx_*) ) = \E [\min (\by,\eta)] 
\end{equation}
where the expectation is taken over the multivariate Gaussian distribution, with mean vector $\mu$ and covariance matrix $\Sigma$, that gives rise after marginalizing the posterior distribution of the GP at $\{\bx_*, \future_{n}(\bx_*)\}$. This integral can be calculated using Expectation Propagation (EP) \cite{Minka:2001} by observing that
