...
\end{eqnarray}
%where $p(y_{t,j}|\bx_{t,j},\I_{t,j-1})= \mathcal{N} \left(y_{t,j};\mu_n(\bx_{t,j};\I_{t,j-1}),\sigma_n^2(\bx_{t,j};\I_{t,j-1} ) \right)$ is the %predictive distribution of the GP at $\bx_{t,j}$ when a total of $n$ points are available and $p(\bx_{t,j}|\I_{t,j-1}) = \delta (\bx_{t,j} - %\arg \max_{\bx \in {\mathcal X}} \alpha(\bx;\I_{t,j-1}) )$ reflects the optimization step required to obtain $\bx_{t,j}$ after the evaluations of $f$ at previous batch-elements have been marginalized. %In Eq. (\ref{eq:optimal_batch}), the `integration' with respect to $\bx_{t,k}$ is explicitly kept as a maximization step, so it is straightforward to identify this expression with the sequential policy when $n_b=1$.
